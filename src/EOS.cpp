		   ...
		   string filename)
{
   ...
   outputFile << "Pressure (Mbar) | Density (g cm^-3) | dP/dRho (Mbar g^-1 cm^3) | d^2P/dRho^2 (Mbar g^-2 cm^6)" << std::endl;
   ...
   while(logP < logEndP)
   {
	 ...
	 outputFile << P/1e11 << " " << rho/1000 << " " 
	 	    << getdPdRho(rho,T)/1.e8 << " " 
	          << getd2PdRho2(rho, T)/1e5;
	 }
...
double EOS::getPMix(double rho, double T)
{
   ...
   // NOTE: failure to converge here will cause EqGen to fail.
...
{
  return ( getP(rho*(1+DIFF_PRECISION),T) - getP(rho,T) ) / (rho*DIFF_PRECISION);
}
